...
def command_thread_func():
    ...
    print("robot_sync on|off         播放时真机同步 MIT+computed torque 跟踪")
    print("robot_goto NAME [T]       真机通过 MIT 轨迹移动到某个示教点")
    print("robot_goto_sim [T]        真机通过 MIT 轨迹移动到当前仿真姿态")
    print("help                      显示命令")
    print("quit                      退出")
    print("==============================================\n")
...
def sample_joint_trajectory(q_targets, duration_per_segment=2.0, sample_rate=200.0):
    points = build_joint_points(q_targets)
...
def connect_robot(device=None):
    ...
    print(f"[robot] connected on {cfg.device}, holding current position")
...
def handle_command(cmd):
    ...
    global recording, video_writer, video_path, playback_active
