#!/usr/bin/env python3
"""
4-arm teleoperation — MIT leaders + PosVel followers.

Usage:
    PYTHONPATH=python python3 python/teleop_posvel.py
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="4-arm teleoperation — MIT leaders + PosVel followers"
    )
...
def sync_followers_to_leaders(
    ...
    for step in range(steps):
        ...
        follower1.send_posvel_command(tc.as_f32(f1_q_des), tc.as_f32(dq_zero))
        follower2.send_posvel_command(tc.as_f32(f2_q_des), tc.as_f32(dq_zero))
...
def main() -> int:
    ...
    try:
        ...
        tc.switch_joints_to_mode(follower1, "follower1", "posvel")
        tc.switch_joints_to_mode(follower2, "follower2", "posvel")
        ...
        print("[teleop] 4-arm teleop running (MIT leaders + PosVel followers), Ctrl+C to stop")
        ...
        while not tc.STOP:
            ...
            follower1.send_posvel_command(tc.as_f32(q1_filt), tc.as_f32(dq_zero))
            follower2.send_posvel_command(tc.as_f32(q2_filt), tc.as_f32(dq_zero))
        ...
        if follower2_started:
            tc.stop_and_disconnect(follower2, "follower2", mode="posvel")
        if follower1_started:
            tc.stop_and_disconnect(follower1, "follower1", mode="posvel")
        if leader2_started:
