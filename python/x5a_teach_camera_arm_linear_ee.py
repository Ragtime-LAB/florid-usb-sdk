...
TRAJ_SAMPLE_RATE = 100.0

TAU_LIMIT = np.array([15.0, 30.0, 30.0, 15.0, 5.0, 5.0], dtype=np.float64)
CT_KP = np.array([260, 180, 280, 220, 210, 80], dtype=np.float64)
CT_KD = np.array([2.8, 3.2, 3.0, 2.3, 2.8, 0.6], dtype=np.float64)
MIT_KP = np.array([18, 20, 20, 20, 14, 16], dtype=np.float64)
MIT_KD = np.array([2.4, 2.0, 2.0, 2.0, 2.2, 1.6], dtype=np.float64)
VEL_FILTER_ALPHA = 0.18
IK_POS_TOL = 0.002
IK_ROT_TOL = 0.01
IK_MAX_ITERS = 80
IK_STEP_ALPHA = 0.35
IK_DLS = 8e-2
IK_STEP_LIMIT = 0.12
TRAJ_SMOOTH_WINDOW = 9
TRAJ_SMOOTH_PASSES = 2
TRAJ_DQ_LIMIT = np.array([1.5, 1.5, 1.5, 1.8, 2.0, 2.5], dtype=np.float64)
TRAJ_DDQ_LIMIT = np.array([4.0, 4.0, 4.0, 5.0, 6.0, 8.0], dtype=np.float64)
...
def command_thread_func():
    ...
    print("list                      查看所有点")
    print("goto NAME [T]             仿真末端线性移动到某个点，默认 T=2.0s")
    print("play NAME1 NAME2 ...      按顺序执行末端线性关键帧回放")
    ...
    print("plot_traj [T]             生成当前末端线性轨迹对应关节曲线图，默认每段 T=2.0s")
    ...
    print("robot_sync on|off         播放时真机同步末端线性 MIT+computed torque 跟踪")
    print("robot_goto NAME [T]       真机通过末端线性 MIT 轨迹移动到某个示教点")
    print("robot_goto_sim [T]        真机通过末端线性 MIT 轨迹移动到当前仿真姿态")
    print("help                      显示命令")
    print("quit                      退出")
    print("==============================================\n")
...
arm_joint_limits = np.array(
    [
        model.jnt_range[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, j)].copy()
...
def quintic_blend(tau, seg_T):
    ...
    ds = (30.0 * u2 - 60.0 * u3 + 30.0 * u4) / seg_T
    dds = (60.0 * u - 180.0 * u2 + 120.0 * u3) / (seg_T**2)
    return s, ds, dds


def q_pin_from_arm_q(q_arm):
...
def arm_q_from_pin(q_pin):
    return np.array([q_pin[pin_q_index[jn]] for jn in ARM_JOINT_NAMES], dtype=np.float64)


def fk_ee_from_arm_q(q_arm):
    q_pin = q_pin_from_arm_q(q_arm)
    pin.forwardKinematics(pin_model, pin_data, q_pin)
    pin.updateFramePlacements(pin_model, pin_data)
    oMe = pin_data.oMf[ee_fid]
    quat = np.zeros(4)
...
def make_waypoint(name, q, ee_pos, ee_quat):
    return {
        "name": str(name),
        "q": np.asarray(q, dtype=np.float64).copy(),
        "ee_pos": np.asarray(ee_pos, dtype=np.float64).copy(),
        "ee_quat_wxyz": quat_normalize(ee_quat),
    }


def current_waypoint(name="current"):
    ee_pos, ee_quat = get_current_ee_pose()
    return make_waypoint(name, data.qpos[arm_qadr].copy(), ee_pos, ee_quat)


def waypoint_from_keyframe(name):
    frame = keyframes[name]
    if "ee_pos" in frame and "ee_quat_wxyz" in frame:
        ee_pos = frame["ee_pos"]
        ee_quat = frame["ee_quat_wxyz"]
    else:
        ee_pos, ee_quat = fk_ee_from_arm_q(np.asarray(frame["q"], dtype=np.float64))
    return make_waypoint(name, frame["q"], ee_pos, ee_quat)


def waypoint_from_q(name, q_target):
    ee_pos, ee_quat = fk_ee_from_arm_q(np.asarray(q_target, dtype=np.float64))
    return make_waypoint(name, q_target, ee_pos, ee_quat)


def solve_ik_pose(target_pos, target_quat, q_seed):
    q_pin = q_pin_from_arm_q(q_seed)
    oMd = pin.SE3(quat_wxyz_to_R(target_quat), np.asarray(target_pos, dtype=np.float64))
    for _ in range(IK_MAX_ITERS):
        ...
        if np.linalg.norm(err6[:3]) < IK_POS_TOL and np.linalg.norm(err6[3:]) < IK_ROT_TOL:
            break
        ...
        J = Jfull[:, arm_vidx]
        JJt = J @ J.T
        dq = J.T @ np.linalg.solve(JJt + IK_DLS * np.eye(6), err6)
        dq = np.clip(dq, -IK_STEP_LIMIT, IK_STEP_LIMIT)
        q_arm = np.clip(arm_q_from_pin(q_pin) + IK_STEP_ALPHA * dq, arm_joint_limits[:, 0], arm_joint_limits[:, 1])
        for idx, jn in enumerate(ARM_JOINT_NAMES):
            q_pin[pin_q_index[jn]] = q_arm[idx]
    return np.clip(arm_q_from_pin(q_pin), arm_joint_limits[:, 0], arm_joint_limits[:, 1])


def smooth_joint_samples(q_arr, locked_indices):
    if len(q_arr) < 3:
        return q_arr.copy()
    ...
    for _ in range(TRAJ_SMOOTH_PASSES):
        next_arr = smoothed.copy()
        for joint_idx in range(smoothed.shape[1]):
            padded = np.pad(smoothed[:, joint_idx], (radius, radius), mode="edge")
            next_arr[:, joint_idx] = np.convolve(padded, kernel, mode="valid")
        next_arr[locked_indices] = q_arr[locked_indices]
        smoothed = next_arr

    return np.clip(smoothed, arm_joint_limits[:, 0], arm_joint_limits[:, 1])


def smooth_signal_samples(arr, locked_indices):
    if len(arr) < 3:
        return arr.copy()
    ...
    for _ in range(TRAJ_SMOOTH_PASSES):
        ...
        for joint_idx in range(smoothed.shape[1]):
            padded = np.pad(smoothed[:, joint_idx], (radius, radius), mode="edge")
            next_arr[:, joint_idx] = np.convolve(padded, kernel, mode="valid")
        next_arr[locked_indices] = arr[locked_indices]
        smoothed = next_arr

    return smoothed


def build_linear_ee_trajectory(
    waypoints, duration_per_segment=2.0, sample_rate=TRAJ_SAMPLE_RATE, start_waypoint=None
):
    if len(waypoints) == 0:
        raise RuntimeError("no target waypoints")
    ...
    seg_idx_list = []
    t_global = 0.0
    start = current_waypoint() if start_waypoint is None else start_waypoint
    q_prev = start["q"].copy()
    prev_waypoint = start
    for seg_idx, target in enumerate(waypoints):
        steps = max(2, int(np.ceil(seg_T / dt)) + 1)
        local_times = np.linspace(0.0, seg_T, steps)
        if seg_idx > 0:
            local_times = local_times[1:]
        for tau in local_times:
            s, _, _ = quintic_blend(tau, seg_T)
            pos = (1.0 - s) * prev_waypoint["ee_pos"] + s * target["ee_pos"]
            quat = quat_slerp(prev_waypoint["ee_quat_wxyz"], target["ee_quat_wxyz"], s)
            q = solve_ik_pose(pos, quat, q_prev)
            t_list.append(t_global + tau)
            q_list.append(q)
            ee_pos_list.append(pos.copy())
            ee_quat_list.append(quat.copy())
            seg_idx_list.append(seg_idx)
            q_prev = q
        t_global += seg_T
        prev_waypoint = target

    q_arr_raw = np.asarray(q_list, dtype=np.float64)
    t_arr = np.asarray(t_list, dtype=np.float64)
    locked_indices = [0, len(q_arr_raw) - 1]
    seg_idx_arr = np.asarray(seg_idx_list)
    for idx in range(1, len(seg_idx_arr)):
        if seg_idx_arr[idx] != seg_idx_arr[idx - 1]:
            locked_indices.extend([idx - 1, idx])
    locked_indices = np.unique(np.asarray(locked_indices, dtype=np.int32))

    q_arr = smooth_joint_samples(q_arr_raw, locked_indices)
    if len(q_arr) == 1:
        dq_arr = np.zeros_like(q_arr)
        ddq_arr = np.zeros_like(q_arr)
    else:
        dq_arr = np.gradient(q_arr, t_arr, axis=0, edge_order=1)
        ddq_arr = np.gradient(dq_arr, t_arr, axis=0, edge_order=1)
        dq_arr = smooth_signal_samples(dq_arr, locked_indices)
        ddq_arr = smooth_signal_samples(ddq_arr, locked_indices)
        dq_arr = np.clip(dq_arr, -TRAJ_DQ_LIMIT, TRAJ_DQ_LIMIT)
        ddq_arr = np.clip(ddq_arr, -TRAJ_DDQ_LIMIT, TRAJ_DDQ_LIMIT)
        dq_arr[locked_indices] = 0.0
        ddq_arr[locked_indices] = 0.0
    return {
        "time": t_arr,
        "q": q_arr,
        "dq": dq_arr,
        ...
        "ee_quat_wxyz": np.asarray(ee_quat_list, dtype=np.float64),
        "segment_index": seg_idx_arr,
...
def sample_precomputed_trajectory(traj, elapsed):
    t = traj["time"]
    if elapsed <= t[0]:
        return traj["q"][0], traj["dq"][0], traj["ddq"][0], int(traj["segment_index"][0])
    if elapsed >= t[-1]:
        idx = len(t) - 1
        return traj["q"][idx], traj["dq"][idx], traj["ddq"][idx], int(traj["segment_index"][idx])
    hi = int(np.searchsorted(t, elapsed, side="right"))
    lo = max(0, hi - 1)
    t0 = t[lo]
    t1 = t[hi]
    if t1 <= t0:
    ...
    q = (1.0 - alpha) * traj["q"][lo] + alpha * traj["q"][hi]
    dq = (1.0 - alpha) * traj["dq"][lo] + alpha * traj["dq"][hi]
    ddq = (1.0 - alpha) * traj["ddq"][lo] + alpha * traj["ddq"][hi]
    seg_idx = int(traj["segment_index"][hi])
    return q, dq, ddq, seg_idx


def sample_joint_trajectory(
    waypoints, duration_per_segment=2.0, sample_rate=TRAJ_SAMPLE_RATE, start_waypoint=None
):
    return build_linear_ee_trajectory(
        waypoints,
...
def cache_playback_path(playback_traj):
    global playback_path_points
    if playback_traj is None or len(playback_traj["ee_pos"]) == 0:
        playback_path_points = np.empty((0, 3), dtype=np.float64)
        return
    playback_path_points = np.asarray(playback_traj["ee_pos"], dtype=np.float64).copy()
...
def reset_robot_actual_path(q_start=None):
    ...
    if q_start is None:
        ...
        return
    ee_pos, _ = fk_ee_from_arm_q(np.asarray(q_start, dtype=np.float64))
    robot_actual_path_points = np.asarray([ee_pos], dtype=np.float64)


def append_robot_actual_path(q_robot):
    global robot_actual_path_points
    ee_pos, _ = fk_ee_from_arm_q(np.asarray(q_robot, dtype=np.float64))
...
def save_trajectory_plot(waypoints, names, duration_per_segment=2.0):
    os.makedirs(PLOT_DIR, exist_ok=True)
    result = sample_joint_trajectory(waypoints, duration_per_segment=duration_per_segment)
    t = result["time"]
    q = result["q"]
    dq = result["dq"]
    ddq = result["ddq"]
...
def compute_feedforward_torque(q_arm, dq_arm, ddq_des):
    q_pin, v_pin = pin_qv_from_robot(q_arm, dq_arm)
    a_pin = np.zeros(pin_model.nv, dtype=np.float64)
    a_pin[arm_vidx] = ddq_des
    tau = pin.rnea(pin_model, pin_data, q_pin, v_pin, a_pin)
    return np.asarray(tau[arm_vidx], dtype=np.float64)


def compute_mass_matrix(q_arm):
    q_pin, _ = pin_qv_from_robot(q_arm, np.zeros(JOINT_COUNT, dtype=np.float64))
    M = pin.crba(pin_model, pin_data, q_pin)
    M = np.asarray(M, dtype=np.float64)
    return M[np.ix_(arm_vidx, arm_vidx)]
...
robot_playback_traj = None
robot_playback_last_seg_idx = -1
...
def disconnect_robot():
    ...
    global robot_playback_active, robot_playback_traj, robot_device
...
def start_robot_playback(waypoints, duration_per_segment, names=None):
    global robot_hold_enabled, robot_hold_q
    global robot_playback_active, robot_playback_traj, robot_playback_last_seg_idx
    ...
    traj = build_linear_ee_trajectory(
        waypoints,
        duration_per_segment=duration_per_segment,
        sample_rate=TRAJ_SAMPLE_RATE,
        start_waypoint=waypoint_from_q("robot_current", q_now),
    )
    if len(traj["time"]) == 0:
        return

    with robot_playback_lock:
        robot_playback_traj = traj
        robot_playback_last_seg_idx = -1
        robot_playback_t0 = time.time()
        robot_playback_duration = float(traj["time"][-1])
        robot_playback_active = True
        robot_hold_enabled = False
        robot_hold_q = traj["q"][-1].copy()
        robot_playback_names = list(names or [])
        robot_playback_log_t0 = time.time()
        robot_playback_logs = init_robot_playback_logs(robot_playback_names, duration_per_segment)
        robot_vel_filt = None
        robot_playback_start_q = q_now.copy()
        robot_last_result = None

    print(f"[robot] MIT playback armed: {traj['segment_count']} segment(s)")
...
def sample_robot_playback(now):
    global robot_playback_active, robot_playback_last_seg_idx, robot_playback_t0
    global robot_hold_enabled, robot_hold_q
    with robot_playback_lock:
        if not robot_playback_active or robot_playback_traj is None:
            return None

        tau = now - robot_playback_t0
        q_des, dq_des, ddq_des, seg_idx = sample_precomputed_trajectory(robot_playback_traj, tau)
        if seg_idx != robot_playback_last_seg_idx:
            robot_playback_last_seg_idx = seg_idx
...
def robot_computed_torque_step(q_des, dq_des, ddq_des):
    ...
    vel_err = dq_des - actual_dq

    gravity_torque = compute_gravity_torque(actual_q)
    feedforward_torque = compute_feedforward_torque(actual_q, actual_dq, ddq_des)
    mass_matrix = compute_mass_matrix(actual_q)
    feedback_acc = CT_KP * pos_err + CT_KD * vel_err
    feedback_torque = mass_matrix @ feedback_acc
...
def move_robot_mit(q_target, duration=REAL_MOVE_DURATION):
    ...
    start_robot_playback(
        [waypoint_from_q("robot_move", q_target)], duration_per_segment=duration, names=["robot_move"]
    )
...
def robot_control_loop():
    ...
    while not robot_thread_stop.is_set():
        ...
        try:
            ...
            if cmd is not None:
                result = robot_computed_torque_step(*cmd)
            ...
            if robot_hold_enabled and robot_hold_q is not None:
                ...
                robot_computed_torque_step(robot_hold_q, zeros, zeros)
...
def start_joint_playback(waypoints, duration_per_segment=2.0, after_mode=MODE_TEACH, names=None):
    global mode, playback_active, playback_traj, playback_last_seg_idx
    ...
    playback_traj = build_linear_ee_trajectory(
        waypoints, duration_per_segment=duration_per_segment, sample_rate=TRAJ_SAMPLE_RATE
    )
    playback_last_seg_idx = -1
    playback_t0 = time.time()
    playback_duration = float(playback_traj["time"][-1])
    playback_after_mode = after_mode
    playback_active = True
    mode = MODE_PLAYBACK
    cache_playback_path(playback_traj)
    print(
        f"[play] start {playback_traj['segment_count']} segment(s), "
        f"duration/seg={playback_traj['duration_per_segment']:.2f}s"
    )
    if robot_sync_enabled:
        start_robot_playback(waypoints, duration_per_segment, names=names)


def update_playback():
    global mode, playback_active, playback_last_seg_idx, playback_t0
    if not playback_active:
        return
    now = time.time()
    tau = now - playback_t0
    q_cmd, _, _, seg_idx = sample_precomputed_trajectory(playback_traj, tau)
    data.qpos[arm_qadr] = q_cmd
    data.qvel[:] = 0.0
    mujoco.mj_forward(model, data)

    if seg_idx != playback_last_seg_idx:
        playback_last_seg_idx = seg_idx
        if seg_idx > 0:
            print(f"[play] segment {seg_idx + 1}/{playback_traj['segment_count']}")
...
def handle_command(cmd):
    ...
    if op == "help":
        ...
        start_joint_playback([waypoint_from_keyframe(name)], duration_per_segment=T, names=[name])
    elif op == "play":
        if len(parts) < 2:
        ...
        if missing:
            ...
            return False
        waypoints = [waypoint_from_keyframe(n) for n in names]
        start_joint_playback(waypoints, duration_per_segment=2.0, names=names)
    elif op == "play_all":
        if not keyframe_order:
            print("[play_all] no keyframes.")
            return False
        waypoints = [waypoint_from_keyframe(n) for n in keyframe_order]
        start_joint_playback(waypoints, duration_per_segment=2.0, names=list(keyframe_order))
    elif op == "delete":
        if len(parts) < 2:
        ...
        waypoints = [waypoint_from_keyframe(n) for n in names]
        save_trajectory_plot(waypoints, names, duration_per_segment=T)
    elif op == "mode":
